...
function getNodeType(node) {
    return node.nodeType || node.ar_;
}

function getNodeValue(node) {
    return node.ax_ || node.nodeValue;
}

function getFirstChild(node) {
    return node.au_ || node.firstChild;
}

function getNextSibling(node) {
    return node.as_ || node.nextSibling;
}
...
function vdomToHTML(node, options) {
    ...
    function serializeElHelper(el, indent) {
        var tagName = el.nodeName || el.ap_;

        var elNamespaceURI = el.namespaceURI || el.bn_;
        ...
        var attributes = el.attributes || el.bm_;
        var attributesArray = [];
        var attrName;
        ...
        if (tagName.toUpperCase() === "TEXTAREA") {
            html += indent + "  VALUE: " + JSON.stringify(ltrim(el.value || el.az_)) + "\n";
        } else {
    ...
    function serializeTextHelper(node, indent) {
        html += indent + JSON.stringify(getNodeValue(node).replace(/\/~vdom\.skip/g, "").replace(/s\d+(-\d+)*/g, "s-normalized")) + "\n";
    }

    function serializeCommentHelper(node, indent) {
        html += indent + "<!--" + JSON.stringify(getNodeValue(node).replace(/s\d+(-\d+)*/g, "s-normalized")) + "-->\n";
    }
