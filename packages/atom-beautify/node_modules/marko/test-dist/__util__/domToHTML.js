...
function vdomToHTML(node, options) {
    ...
    function serializeHelper(node) {
        var nodeType = node.nodeType || node.ar_;
    ...
    function serializeElHelper(el) {
        ...
        if (tagName.toUpperCase() === "TEXTAREA") {
            ...
            if (curChild) {
                while (curChild) {
                    let nodeType = curChild.nodeType || curChild.ar_;
                    if (nodeType === 3) {
                        let tag = tagName.toUpperCase();
                        let escapeText = tag === "SCRIPT" ? escapeScript : tag === "STYLE" ? escapeStyle : escapeXml;
    ...
    let nodeType = node.nodeType || node.ar_;
