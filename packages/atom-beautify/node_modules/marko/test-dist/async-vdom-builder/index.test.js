...
describe("AsyncVDOMBuilder", function () {
    ...
    it("async", function (done) {
        ...
        out.on("finish", function (result) {
            ...
            expect(tree.au_.ap_).to.equal("div");
            expect(tree.au_.as_.ap_).to.equal("span");
            expect(tree.au_.as_.as_.ap_).to.equal("section");
            done();
        });
    });
    ...
    it("for loop", function (done) {
        ...
        out.once("finish", function (result) {
            ...
            expect(header.ap_).to.equal("h1");
            expect(list.ap_).to.equal("ul");
            expect(getChildNodes(list).length).to.equal(10);
            expect(paragraph.ap_).to.equal("p");
            done();
        });
    ...
    it("staticNode, text, comment", function (done) {
        ...
        out.once("finish", function (result) {
            var tree = result.getOutput();
            var childNodes = getChildNodes(tree);
            expect(childNodes[0].ap_).to.equal("div");
            expect(childNodes[1].ax_).to.equal("Hello <em>World</em>");
            expect(childNodes[2].ax_).to.equal("TODO: make this work");
            done();
        });
    });
